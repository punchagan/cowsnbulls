...
Elm.CowsNBulls = Elm.CowsNBulls || {};
Elm.CowsNBulls.make = function (_elm) {
   "use strict";
   _elm.CowsNBulls = _elm.CowsNBulls || {};
   if (_elm.CowsNBulls.values)
   return _elm.CowsNBulls.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "CowsNBulls",
   $Basics = Elm.Basics.make(_elm),
   ...
   var getRandomItem = F2(function (seed,
   xs) {
      return function () {
   ...
   var validWord = F2(function (guess,
   words) {
      return A2($List.member,
      $String.toLower(guess),
      words);
   });
   var checkGuess = F2(function (word,
   guess) {
      return function () {
         var bulls = $String.length(A2($String.filter,
         function (x) {
            return function () {
               var y = $String.fromChar(x);
               return _U.eq(A2($String.indexes,
               y,
               word),
               A2($String.indexes,y,guess));
            }();
         },
         guess));
         var word_set = $Set.fromList($String.toList(word));
         var guess_set = $Set.fromList($String.toList(guess));
         var total = $List.length($Set.toList(A2($Set.intersect,
         word_set,
         guess_set)));
         return {ctor: "_Tuple2"
                ,_0: bulls
   ...
   var noWordList = $Signal.mailbox(false);
   var pickWord = $Signal.mailbox(1);
   var clockSeed = A2($Signal._op["<~"],
   function (time) {
      return $Random.initialSeed($Basics.round(time));
   },
   $Time.every($Time.millisecond));
   var wordDb = $Signal.mailbox(_L.fromArray([]));
      ...
      return function () {
         ...
         {case "Just":
            ...
            return A2($Signal.send,
              ...
              true);}
         _U.badCase($moduleName,
         "between lines 391 and 397");
      }();
   };
   var updateWords = function (content) {
      return function () {
         switch (content.ctor)
         {case "Just":
            return function () {
                 var words = $String.lines($String.trim(content._0));
                 return A2($Task.andThen,
                 setWords($Maybe.Just(words)),
                 function (_v9) {
                    return function () {
                       return $Storage.setItem("words")($Json$Encode.list(A2($List.map,
                       $Json$Encode.string,
                       words)));
                    }();
                 });
            ...
            case "Nothing":
            return A2($Task.andThen,
              $Task.toMaybe($Storage.getItem("words")($Json$Decode.list($Json$Decode.string))),
              setWords);}
         _U.badCase($moduleName,
         "between lines 378 and 387");
      }();
   };
   var getWords = Elm.Native.Task.make(_elm).perform(function () {
      var url = "word_list_0a7bf29.txt";
      return A2($Task.andThen,
   ...
   var viewButton = F4(function (address,
   action,
   extraTag,
   label) {
   ...
   var viewCard = function (contents) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("row")]),
      _L.fromArray([A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("col-xs-6 col-xs-offset-3")]),
      _L.fromArray([A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("panel panel-default")]),
      contents)]))]));
   };
   var viewHistoryCard = F2(function (address,
   model) {
      return function () {
         var historyTableRow = function (_v13) {
            return function () {
               switch (_v13.ctor)
               {case "_Tuple2":
                  switch (_v13._1.ctor)
                    {case "_Tuple2":
                       return A2($Html.tr,
                         ...
                         _L.fromArray([A2($Html.td,
                                      _L.fromArray([]),
                                      _L.fromArray([$Html.text(_v13._0)]))
                                      ,A2($Html.td,
                                      _L.fromArray([]),
                                      _L.fromArray([$Html.text($Basics.toString(_v13._1._0))]))
                                      ,A2($Html.td,
                                      _L.fromArray([]),
                                      _L.fromArray([$Html.text($Basics.toString(_v13._1._1))]))]));}
                    break;}
               _U.badCase($moduleName,
               "between lines 313 and 318");
            }();
         };
         var historyTableBody = A2($Html.tbody,
         _L.fromArray([]),
         A2($List.map,
         historyTableRow,
         model.guesses));
         var historyTableHead = A2($Html.thead,
         _L.fromArray([]),
         _L.fromArray([A2($Html.tr,
         _L.fromArray([$Html$Attributes.$class("active")]),
         _L.fromArray([A2($Html.th,
                      _L.fromArray([]),
                      _L.fromArray([$Html.text("Guess")]))
                      ,A2($Html.th,
                      _L.fromArray([]),
                      _L.fromArray([$Html.text("Bulls")]))
                      ,A2($Html.th,
                      _L.fromArray([]),
                      _L.fromArray([$Html.text("Cows")]))]))]));
         var historyTable = A2($Html.table,
         _L.fromArray([$Html$Attributes.$class("table table-hover")]),
         _L.fromArray([historyTableHead
                      ,historyTableBody]));
         var historyBody = A2($Html.div,
         _L.fromArray([$Html$Attributes.$class("table-responsive")]),
         _L.fromArray([historyTable]));
         var title = A2($Html.h3,
         _L.fromArray([$Html$Attributes.$class("panel-title pull-left")]),
         _L.fromArray([$Html.text("Your guesses")]));
         var historyHeading = A2($Html.div,
         _L.fromArray([$Html$Attributes.$class("panel-heading clearfix")]),
         _L.fromArray([title]));
         return viewCard(_L.fromArray([historyHeading
                                      ,historyBody]));
      }();
   });
   var siteFooter = A2($Html.footer,
   _L.fromArray([$Html$Attributes.$class("navbar navbar-default navbar-fixed-bottom")
   ...
   var ShowWord = {ctor: "ShowWord"};
   var PickWord = function (a) {
      return {ctor: "PickWord"
             ,_0: a};
   };
   var UpdateWords = function (a) {
      return {ctor: "UpdateWords"
             ,_0: a};
   };
   ...
   var Guess = {ctor: "Guess"};
   var NoOp = {ctor: "NoOp"};
   var actions = $Signal.mailbox(NoOp);
   var updateEvents = $Signal.mergeMany(_L.fromArray([actions.signal
                                                     ,A2($Signal.map,
                                                     UpdateWords,
                                                     wordDb.signal)
                                                     ,A2($Signal.map,
   ...
   var showMessage = function (message) {
      return function () {
         ...
         {case "DownloadMessage":
            return "Wait, Downloading word list...";
            case "ErrorMessage":
            return "Only \'valid\' 4-letter, without repetition! Should we add word to our list?";
            case "GuessMessage":
            return "Next guess?";
            case "GuessedMessage":
            return "You have already guessed this!";
            case "NoWordListMessage":
            return "Failed to download word list.";
            case "StartMessage":
            return "I thought of a 4-letter word. Guess";}
         _U.badCase($moduleName,
         "between lines 33 and 39");
      }();
   };
   var viewInputCard = F2(function (address,
   model) {
      return function () {
         var guessCount = function (count) {
            return A2($Basics._op["++"],
            $Basics.toString(count),
            _U.eq(count,
            1) ? " guess" : " guesses");
         };
         var successMessage = A2($Basics._op["++"],
         "Hooray! Guessed \'",
         A2($Basics._op["++"],
         model.word,
         A2($Basics._op["++"],
         "\' in ",
         guessCount(model.count))));
         var tweetMessage = A2($Basics._op["++"],
         "Hooray! I Guessed \'",
         A2($Basics._op["++"],
         model.word,
         A2($Basics._op["++"],
         ...
         A2($Basics._op["++"],
         guessCount(model.count),
         " at http://cowsnbulls.in"))));
         var tweetLink = A2($Html.a,
         _L.fromArray([$Html$Attributes.href(A2($Basics._op["++"],
                      "https://twitter.com/home?status=",
                      tweetMessage))
                      ,$Html$Attributes.target("_blank")]),
         _L.fromArray([A2($Html.img,
         _L.fromArray([$Html$Attributes.src("https://cdn3.iconfinder.com/data/icons/rcons-social/32/bird_twitter-32.png")]),
         _L.fromArray([]))]));
         var success = A2($Html.div,
         _L.fromArray([]),
         _L.fromArray([A2($Html.p,
                      _L.fromArray([]),
                      _L.fromArray([$Html.text(successMessage)]))
                      ,A2($Html.p,
                      _L.fromArray([]),
                      _L.fromArray([tweetLink]))]));
         var dictLookup = A2($Html.p,
         _L.fromArray([]),
         _L.fromArray([$Html.text("See meaning: ")
                      ,A2($Html.a,
                      _L.fromArray([$Html$Attributes.href(A2($Basics._op["++"],
                                   "http://www.dict.org/bin/Dict?Form=Dict2&Database=*&Query=",
                                   model.word))
                                   ,$Html$Attributes.target("_blank")]),
                      _L.fromArray([$Html.text(model.word)]))]));
         var doneBody = A2($Html.div,
         _L.fromArray([$Html$Attributes.$class("panel-body")]),
         model.done ? _L.fromArray([success
                                   ,dictLookup]) : _L.fromArray([dictLookup]));
         var guesses = A2($Html.div,
         _L.fromArray([$Html$Attributes.$class("list-group-item")]),
         _L.fromArray([A2($Html.p,
                      _L.fromArray([$Html$Attributes.$class("list-group-item-text")]),
                      _L.fromArray([$Html.text("Guesses")]))
                      ,A2($Html.h4,
                      _L.fromArray([$Html$Attributes.$class("list-group-item-heading")]),
                      _L.fromArray([$Html.text($Basics.toString(model.count))]))]));
         var lastScore = function (_v20) {
            return function () {
               switch (_v20.ctor)
               {case "_Tuple2":
                  return A2($Basics._op["++"],
                    $Basics.toString(_v20._0),
                    A2($Basics._op["++"],
                    " bulls, ",
                    A2($Basics._op["++"],
                    $Basics.toString(_v20._1),
                    " cows")));}
               _U.badCase($moduleName,
               "on line 235, column 32 to 81");
            }();
         };
         var score = A2($Html.div,
         _L.fromArray([$Html$Attributes.$class("list-group-item")]),
         _L.fromArray([A2($Html.p,
                      _L.fromArray([$Html$Attributes.$class("list-group-item-text")]),
                      _L.fromArray([$Html.text(model.guess)]))
                      ,A2($Html.h4,
                      _L.fromArray([$Html$Attributes.$class("list-group-item-heading")]),
                      _L.fromArray([$Html.text(lastScore(model.result))]))]));
         var scoreBody = A2($Html.div,
         _L.fromArray([$Html$Attributes.$class("list-group")]),
         _L.fromArray([score,guesses]));
         var inputGuessField = A2($Html.input,
         _L.fromArray([$Html$Attributes.id("guess")
                      ,$Html$Attributes.$class("form-control")
                      ,$Html$Attributes.placeholder(showMessage(model.message))
                      ...
                      function ($) {
                         return $Signal.message(address)(UpdateInput($));
                      })
                      ,A2(onEnter,address,Guess)]),
         _L.fromArray([]));
         var inputGuess = A2($Html.div,
         _L.fromArray([$Html$Attributes.$class("form-group")]),
         _L.fromArray([A2($Html.div,
         _L.fromArray([$Html$Attributes.$class("col-xs-12")]),
         _L.fromArray([inputGuessField]))]));
         var guessForm = A2($Html.div,
         _L.fromArray([$Html$Attributes.$class("form-horizontal")]),
         _L.fromArray([inputGuess]));
         var inputBody = A2($Html.div,
         _L.fromArray([$Html$Attributes.$class("panel-body")]),
         _L.fromArray([guessForm]));
         var restartButton = A2($Html.button,
         _L.fromArray([A2($Html$Events.onClick,
                      pickWord.address,
                      1)
                      ,$Html$Attributes.$class("btn pull-right btn-primary")]),
         _L.fromArray([$Html.text("Restart")]));
         var giveUpButton = A4(viewButton,
         address,
         ShowWord,
         "btn-danger",
         "Give Up");
         var showHistoryButton = A4(viewButton,
         address,
         ToggleShowHistory,
         "btn-warning",
         "Show Guesses");
         ...
         0) > 0) ? _L.fromArray([title
                                ,scoreButton]) : _L.fromArray([title]));
         return viewCard(A2($Basics._op["++"],
         _L.fromArray([inputHeading]),
         A2($Basics._op["++"],
         _L.fromArray([model.done || model.cancelled ? doneBody : inputBody]),
         _U.cmp(model.count,
         0) > 0 ? _L.fromArray([scoreBody]) : _L.fromArray([]))));
      }();
   });
   ...
   model) {
      return A2($Html.div,
      _L.fromArray([]),
      _L.fromArray([A2(viewInputCard,
                   address,
                   model)
                   ,model.show_history ? A2(viewHistoryCard,
                   address,
                   model) : A2($Html.div,
                   _L.fromArray([]),
                   _L.fromArray([]))
   ...
   var GuessedMessage = {ctor: "GuessedMessage"};
   var NoWordListMessage = {ctor: "NoWordListMessage"};
   var ErrorMessage = {ctor: "ErrorMessage"};
   var GuessMessage = {ctor: "GuessMessage"};
   var StartMessage = {ctor: "StartMessage"};
   var DownloadMessage = {ctor: "DownloadMessage"};
   var emptyModel = {_: {}
                    ,cancelled: false
                    ,count: 0
                    ,done: false
                    ,guess: ""
                    ,guesses: _L.fromArray([])
                    ,input: ""
   ...
   model) {
      return function () {
         ...
         {case "Guess":
            return function () {
                 var result = A2(checkGuess,
                 $String.toLower(model.word),
                 $String.toLower(model.input));
                 var guess = $String.toLower(model.input);
                 var guessed = A2($List.member,
                 guess,
                 A2($List.map,
                 function (_v29) {
                    return function () {
                       ...
                       {case "_Tuple2":
                          return _v29._0;}
                       _U.badCase($moduleName,
                       "on line 95, column 65 to 66");
                    }();
                 },
                 model.guesses));
                 var valid = A2(validWord,
                 model.input,
                 model.words);
                 return valid && $Basics.not(guessed) ? _U.replace([["guess"
                                                                    ,guess]
                                                                   ,["result"
                                                                    ,result]
                                                                   ,["input",""]
                                                                   ...
                                                                   ,["guesses"
                                                                    ...
                                                                    _L.fromArray([{ctor: "_Tuple2"
                                                                                  ,_0: guess
                                                                                  ,_1: result}]))]],
                 model) : guessed ? _U.replace([["input"
                                                ,""]
                                               ,["message",GuessedMessage]],
                 model) : _U.replace([["input"
                                      ,""]
            ...
            return _U.replace([["message"
              ...
              emptyModel);
            case "PickWord":
            return _U.replace([["word"
                               ,$Maybe.withDefault("")(A2(getRandomItem,
                               action._0,
                               model.words))]
                              ,["words",model.words]
              ...
              emptyModel);
            case "ShowWord":
            return _U.replace([["cancelled"
                               ,true]
                              ,["count",0]],
              model);
            case "ToggleShowHistory":
            return _U.replace([["show_history"
                               ,$Basics.not(model.show_history)]],
              model);
            case "UpdateInput":
            return _U.replace([["input"
                               ,$String.toLower(action._0)]
                              ,["done",false]],
              model);
            case "UpdateWords":
   ...
   model);
   _elm.CowsNBulls.values = {_op: _op
                            ,DownloadMessage: DownloadMessage
                            ,StartMessage: StartMessage
                            ,GuessMessage: GuessMessage
                            ,ErrorMessage: ErrorMessage
                            ,NoWordListMessage: NoWordListMessage
                            ,GuessedMessage: GuessedMessage
                            ...
                            ,PickWord: PickWord
                            ,ShowWord: ShowWord
                            ,NoWordList: NoWordList
                            ,ToggleShowHistory: ToggleShowHistory
                            ,update: update
                            ,siteFooter: siteFooter
                            ,viewCard: viewCard
                            ,viewButton: viewButton
                            ,viewInputCard: viewInputCard
                            ,viewHistoryCard: viewHistoryCard
                            ,view: view
                            ,onEnter: onEnter
                            ,is13: is13
                            ,main: main
...
Elm.Storage.make = function (_elm) {
   ...
   if (_elm.Storage.values)
   return _elm.Storage.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
